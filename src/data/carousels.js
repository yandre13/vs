const carousel1 = {
	logo: '',
	images: [
	  {
		name: 'Acercamiento',
		path: '../../img/proyectos/MÁLAGA_BALTAZAR/ACERCAMIENTO.webp',
		classnames: '',
	  },
	  {
		name: 'Baño',
		path: '../../img/proyectos/MÁLAGA_BALTAZAR/BAÑO(4).webp',
		classnames: '',
	  },
	  {
		name: 'Cocina',
		path: '../../img/proyectos/MÁLAGA_BALTAZAR/COCINA(14).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio',
		path: '../../img/proyectos/MÁLAGA_BALTAZAR/DORMITORIO(5).webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Exterior',
		path: '../../img/proyectos/MÁLAGA_BALTAZAR/FACHADAEXTERIOR.webp',
		classnames: '',
	  },
	  {
		name: 'Patio',
		path: '../../img/proyectos/MÁLAGA_BALTAZAR/PATIO(4).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/MÁLAGA_BALTAZAR/SALACOMEDOR(13).webp',
		classnames: '',
	  },
	]
  };

  const carousel2 = {
	logo: '../../public/img/marcas/EDIFICA.png',
	images: [
	  {
		name: 'Lobby 1',
		path: '../../img/proyectos/EDIFICA_ACACIAS/LOBBY1(1).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby 2',
		path: '../../img/proyectos/EDIFICA_ACACIAS/LOBBY2(2).webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Espera',
		path: '../../img/proyectos/EDIFICA_ACACIAS/SALADEESPERA(2).webp',
		classnames: '',
	  },
	]
  };

  const carousel3 = {
	logo: '../../public/img/marcas/FD_ARQ.jpg',
	images: [
	  {
		name: 'Fachada Externa Final',
		path: '../../img/proyectos/FD ARQUITECTOS_REX/FACHADAEXTERNAFINAL.webp',
		classnames: '',
	  },
	  {
		name: 'Plaza Interior Actualizada Final',
		path: '../../img/proyectos/FD ARQUITECTOS_REX/PLAZAINTERNAACTUALIZADAFINAL.webp',
		classnames: '',
	  },
	  {
		name: 'V1',
		path: '../../img/proyectos/FD ARQUITECTOS_REX/V1(3).webp',
		classnames: '',
	  },
	  {
		name: 'V2',
		path: '../../img/proyectos/FD ARQUITECTOS_REX/V2(5).webp',
		classnames: '',
	  },
	]
  };
  
const carousel16 = {
	logo: '../../public/img/marcas/ABRIL.png',
	images: [
	  {
		name: 'Baño Principal',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_CATLEYA/1201BAÑOPRINCIPAL.webp',
		classnames: '',
	  },
	  {
		name: 'Kitchenette',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_CATLEYA/1201KITCHENETTE.webp',
		classnames: '',
	  },
	  {
		name: 'Detalle',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_CATLEYA/DETALLE.webp',
		classnames: '',
	  },
	  {
		name: 'Frontal',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_CATLEYA/FRONTAL.webp',
		classnames: '',
	  },
	  {
		name: 'Gimnasio',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_CATLEYA/GYM(3).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_CATLEYA/LOBBY(7).webp',
		classnames: '',
	  },
	  {
		name: 'Lounge Bar',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_CATLEYA/LOUNGEBAR.webp',
		classnames: '',
	  },
	  {
		name: 'Nocturna',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_CATLEYA/NOCTURNA(2).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel44 = {
	logo: '../../public/img/marcas/ABRIL.png',
	images: [
	  {
		name: 'Baño Secundario',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/1502ABAÑOSECUNDARIO.webp',
		classnames: '',
	  },
	  {
		name: 'Baño Visitas',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/1502ABAÑOVISITAS.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/1502ACOCINA.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Secundario',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/1502ADORMITORIOSECUNDARIO.webp',
		classnames: '',
	  },
	  {
		name: 'Sala',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/1502ASALA.webp',
		classnames: '',
	  },
	  {
		name: 'Kitchenette',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/1503BKITCHENETTE.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/1506BSALACOMEDOR.webp',
		classnames: '',
	  },
	  {
		name: 'Detalle',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/DETALLE(1).webp',
		classnames: '',
	  },
	  {
		name: 'Diurna',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/DIURNA(1).webp',
		classnames: '',
	  },
	  {
		name: 'Gimnasio',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/GIMNASIO.webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/LOBBY(8).webp',
		classnames: '',
	  },
	  {
		name: 'Nocturna',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/NOCTURNA(3).webp',
		classnames: '',
	  },
	  {
		name: 'Parrilla',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/PARRILLA(2).webp',
		classnames: '',
	  },
	  {
		name: 'Piscina',
		path: '../../img/proyectos/ABRIL INMOBILIARIA_KAVA/PISCINA(12).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel19 = {
	logo: '../../public/img/marcas/ARMANDO_PAREDES.jpg',
	images: [
	  {
		name: 'Baño UYM',
		path: '../../img/proyectos/ARMANDO PAREDES_ UGARTE Y MOSCOSO/BAÑOUYM.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina UYM',
		path: '../../img/proyectos/ARMANDO PAREDES_ UGARTE Y MOSCOSO/COCINA_UYM.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio UYM',
		path: '../../img/proyectos/ARMANDO PAREDES_ UGARTE Y MOSCOSO/DORMITORIOUYM.webp',
		classnames: '',
	  },
	  {
		name: 'Fachada UYM',
		path: '../../img/proyectos/ARMANDO PAREDES_ UGARTE Y MOSCOSO/FACHADAUYM(2).webp',
		classnames: '',
	  },
	  {
		name: 'Sala UYM',
		path: '../../img/proyectos/ARMANDO PAREDES_ UGARTE Y MOSCOSO/SALAUYM(1).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel35 = {
	logo: '../../public/img/marcas/ARMANDO_PAREDES.jpg',
	images: [
	  {
		name: 'Cocina 106',
		path: '../../img/proyectos/ARMANDO PAREDES_INCARIO/COCINA106.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina 604',
		path: '../../img/proyectos/ARMANDO PAREDES_INCARIO/COCINA604.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/ARMANDO PAREDES_INCARIO/DORMITORIOPRINCIPAL(1).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/ARMANDO PAREDES_INCARIO/SALACOMEDOR(8).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel24 = {
	logo: '../../public/img/marcas/ARMANDO_PAREDES.jpg',
	images: [
	  {
		name: 'Boutique',
		path: '../../img/proyectos/ARMANDO PAREDES_LIBERTAD/BOUTIQUE(1).webp',
		classnames: '',
	  },
	  {
		name: 'C2',
		path: '../../img/proyectos/ARMANDO PAREDES_LIBERTAD/C2.webp',
		classnames: '',
	  },
	  {
		name: 'Estudio Arq',
		path: '../../img/proyectos/ARMANDO PAREDES_LIBERTAD/ESTUDIOARQ(1).webp',
		classnames: '',
	  },
	  {
		name: 'Estudio Fotográfico',
		path: '../../img/proyectos/ARMANDO PAREDES_LIBERTAD/ESTUDIOFOTOGRÁFICO.webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Final',
		path: '../../img/proyectos/ARMANDO PAREDES_LIBERTAD/FACHADA_V1FINALRGB.webp',
		classnames: '',
	  },
	  {
		name: 'Lobby Final',
		path: '../../img/proyectos/ARMANDO PAREDES_LIBERTAD/LIB_FINALENALTA_LOBBY.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel43 = {
	logo: '../../public/img/marcas/ARMANDO_PAREDES.jpg',
	images: [
	  {
		name: 'F5',
		path: '../../img/proyectos/ARMANDO PAREDES_SAENZ PEÑA/F5.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel47 = {
	logo: '../../public/img/marcas/ARMANDO_PAREDES.jpg',
	images: [
	  {
		name: 'SUM',
		path: '../../img/proyectos/ARMANDO PAREDES_TORIBIO/SUM(1).webp',
		classnames: '',
	  },
	  {
		name: 'Tor Cocina',
		path: '../../img/proyectos/ARMANDO PAREDES_TORIBIO/TOR_COCINA.webp',
		classnames: '',
	  },
	  {
		name: 'Tor Dormitorio Principal',
		path: '../../img/proyectos/ARMANDO PAREDES_TORIBIO/TOR_DORMITORIOPRINCIPAL.webp',
		classnames: '',
	  },
	  {
		name: 'Tor Dormitorio Secundario',
		path: '../../img/proyectos/ARMANDO PAREDES_TORIBIO/TOR_DORMITORIOSECUNDARIO.webp',
		classnames: '',
	  },
	  {
		name: 'Vista Atardecer',
		path: '../../img/proyectos/ARMANDO PAREDES_TORIBIO/VISTAATARDECER.webp',
		classnames: '',
	  },
	  {
		name: 'Zona de Niños',
		path: '../../img/proyectos/ARMANDO PAREDES_TORIBIO/ZONADENIÑOS.webp',
		classnames: '',
	  },
	  {
		name: 'Zona Parrilla',
		path: '../../img/proyectos/ARMANDO PAREDES_TORIBIO/ZONAPARRILLA.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel38 = {
	logo: '../../public/img/marcas/ARMANDO_PAREDES.jpg',
	images: [
	  {
		name: 'Cocina',
		path: '../../img/proyectos/ARMHONY_MAR DE PLATA/COCINA(8).webp',
		classnames: '',
	  },
	  {
		name: 'Cropped Fachada',
		path: '../../img/proyectos/ARMHONY_MAR DE PLATA/CROPFACHADA.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/ARMHONY_MAR DE PLATA/DORMPRINC.webp',
		classnames: '',
	  },
	  {
		name: 'F4',
		path: '../../img/proyectos/ARMHONY_MAR DE PLATA/F4.webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Diurna',
		path: '../../img/proyectos/ARMHONY_MAR DE PLATA/FACHADADIURNA.webp',
		classnames: '',
	  },
	  {
		name: 'Patio Interior',
		path: '../../img/proyectos/ARMHONY_MAR DE PLATA/PATIOINTERIOR.webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Estar',
		path: '../../img/proyectos/ARMHONY_MAR DE PLATA/SALADEESTAR(1).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel9 = {
	logo: '../../public/img/marcas/ASCOLUX.png',
	images: [
	  {
		name: 'Terrazza',
		path: '../../img/proyectos/ASCOLUX_BREGASSONA/09TERRAZZA.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/ASCOLUX_BREGASSONA/DORMITORIOPRINCIPALE.webp',
		classnames: '',
	  },
	  {
		name: 'Dorm Principal App12',
		path: '../../img/proyectos/ASCOLUX_BREGASSONA/DORMPRINC_APP12.webp',
		classnames: '',
	  },
	  {
		name: 'Sala',
		path: '../../img/proyectos/ASCOLUX_BREGASSONA/SALA(11).webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Pranzo',
		path: '../../img/proyectos/ASCOLUX_BREGASSONA/SALADAPRANZO.webp',
		classnames: '',
	  },
	  {
		name: 'Soggiorno 1',
		path: '../../img/proyectos/ASCOLUX_BREGASSONA/SOGGIORNO(1).webp',
		classnames: '',
	  },
	  {
		name: 'Soggiorno 2',
		path: '../../img/proyectos/ASCOLUX_BREGASSONA/SOGGIORNO.webp',
		classnames: '',
	  },
	  {
		name: 'Terrazza 2',
		path: '../../img/proyectos/ASCOLUX_BREGASSONA/TERRAZZA2.webp',
		classnames: '',
	  },
	  {
		name: 'Vista Aérea',
		path: '../../img/proyectos/ASCOLUX_BREGASSONA/VISTAAEREA.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel33 = {
	logo: '../../public/img/marcas/CADIZ.png',
	images: [
	  {
		name: 'Baño Principal',
		path: '../../img/proyectos/CADIZ INMB_MÁLAGA/BAÑOPRINC.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina 2',
		path: '../../img/proyectos/CADIZ INMB_MÁLAGA/COCINA2.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/CADIZ INMB_MÁLAGA/DORMITORIOPRINCIPAL(2).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/CADIZ INMB_MÁLAGA/LOBBY(7).webp',
		classnames: '',
	  },
	  {
		name: 'Loft 401-1',
		path: '../../img/proyectos/CADIZ INMB_MÁLAGA/LOFT401-1.webp',
		classnames: '',
	  },
	  {
		name: 'Sala',
		path: '../../img/proyectos/CADIZ INMB_MÁLAGA/SALA(6).webp',
		classnames: '',
	  },
	  {
		name: 'Vista Diurna',
		path: '../../img/proyectos/CADIZ INMB_MÁLAGA/VISTADIURNA(1).webp',
		classnames: '',
	  },
	  {
		name: 'Vista Nocturna',
		path: '../../img/proyectos/CADIZ INMB_MÁLAGA/VISTANOCTURNA(4).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel23 = {
	logo: '../../public/img/marcas/CARETCAPITAL.png',
	images: [
	  {
		name: 'Baño',
		path: '../../img/proyectos/CARET CAPITAL_BENICALAP/BAÑO(3).webp',
		classnames: '',
	  },
	  {
		name: 'Cocina',
		path: '../../img/proyectos/CARET CAPITAL_BENICALAP/COCINA(9).webp',
		classnames: '',
	  },
	  {
		name: 'Cocina Comedor',
		path: '../../img/proyectos/CARET CAPITAL_BENICALAP/COCINACOMEDOR(1).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal 1',
		path: '../../img/proyectos/CARET CAPITAL_BENICALAP/DORMITORIOPRINCIPAL(3).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal 2',
		path: '../../img/proyectos/CARET CAPITAL_BENICALAP/DORMITORIOPRINCIPAL(4).webp',
		classnames: '',
	  },
	  {
		name: 'Fachada',
		path: '../../img/proyectos/CARET CAPITAL_BENICALAP/FACHADA(6).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/CARET CAPITAL_BENICALAP/LOBBY(8).webp',
		classnames: '',
	  },
	  {
		name: 'Sala',
		path: '../../img/proyectos/CARET CAPITAL_BENICALAP/SALA(7).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel34 = {
	logo: '../../public/img/marcas/CARETCAPITAL.png',
	images: [
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/CARET CAPITAL_VASQUEZ DE MELLA/DORMPRI.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Secundario',
		path: '../../img/proyectos/CARET CAPITAL_VASQUEZ DE MELLA/DORMSEC(1).webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Frontal',
		path: '../../img/proyectos/CARET CAPITAL_VASQUEZ DE MELLA/FACHADAFRONTAL(1)(1).webp',
		classnames: '',
	  },
	  {
		name: 'Home Office',
		path: '../../img/proyectos/CARET CAPITAL_VASQUEZ DE MELLA/HOMEOFFICE.webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/CARET CAPITAL_VASQUEZ DE MELLA/LOBBY(4).webp',
		classnames: '',
	  },
	  {
		name: 'Sala 1',
		path: '../../img/proyectos/CARET CAPITAL_VASQUEZ DE MELLA/SALA1.webp',
		classnames: '',
	  },
	  {
		name: 'Sala 2',
		path: '../../img/proyectos/CARET CAPITAL_VASQUEZ DE MELLA/SALA2.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/CARET CAPITAL_VASQUEZ DE MELLA/SALACOMEDORC.webp',
		classnames: '',
	  },
	  {
		name: 'Terraza',
		path: '../../img/proyectos/CARET CAPITAL_VASQUEZ DE MELLA/TERRAZA(3).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel40 = {
	logo: '../../public/img/marcas/CHECOR.png',
	images: [
	  {
		name: 'Baño Dalias',
		path: '../../img/proyectos/CHECOR_DALIAS/BANO_DALIAS.webp',
		classnames: '',
	  },
	  {
		name: 'F1',
		path: '../../img/proyectos/CHECOR_DALIAS/F1.webp',
		classnames: '',
	  },
	  {
		name: 'Sala 1 Dalias',
		path: '../../img/proyectos/CHECOR_DALIAS/SALA1_DALIAS.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Cocina 2 Dalias',
		path: '../../img/proyectos/CHECOR_DALIAS/SALA_COCINA2_DALIAS.webp',
		classnames: '',
	  },
	  {
		name: 'SUM 3',
		path: '../../img/proyectos/CHECOR_DALIAS/SUM_3.webp',
		classnames: '',
	  },
	  {
		name: 'Zona Gourmet',
		path: '../../img/proyectos/CHECOR_DALIAS/ZONAGOURMET.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel17 = {
	logo: '../../public/img/marcas/CHECOR.png',
	images: [
	  {
		name: 'Bike Zone',
		path: '../../img/proyectos/CHECOR_LIRIOS/BIKEZONE.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina X01',
		path: '../../img/proyectos/CHECOR_LIRIOS/COCINAX01.webp',
		classnames: '',
	  },
	  {
		name: 'Coworking',
		path: '../../img/proyectos/CHECOR_LIRIOS/COWORKING(4).webp',
		classnames: '',
	  },
	  {
		name: 'Diurna',
		path: '../../img/proyectos/CHECOR_LIRIOS/DIA(2).webp',
		classnames: '',
	  },
	  {
		name: 'Gourmet',
		path: '../../img/proyectos/CHECOR_LIRIOS/GOURMET.webp',
		classnames: '',
	  },
	  {
		name: 'Gym',
		path: '../../img/proyectos/CHECOR_LIRIOS/GYM(3).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/CHECOR_LIRIOS/LOBBY(9).webp',
		classnames: '',
	  },
	  {
		name: 'Nocturna',
		path: '../../img/proyectos/CHECOR_LIRIOS/NOCHE(1).webp',
		classnames: '',
	  },
	  {
		name: 'Terraza',
		path: '../../img/proyectos/CHECOR_LIRIOS/TERRAZA(5).webp',
		classnames: '',
	  },
	  {
		name: 'Baño X01',
		path: '../../img/proyectos/CHECOR_LIRIOS/X01BAÑO.webp',
		classnames: '',
	  },
	  {
		name: 'Sala X01',
		path: '../../img/proyectos/CHECOR_LIRIOS/X01SALA.webp',
		classnames: '',
	  },
	  {
		name: 'Zona de Espera',
		path: '../../img/proyectos/CHECOR_LIRIOS/ZONAESPERA.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel50 = {
	logo: '../../public/img/marcas/CHECOR.png',
	images: [
	  {
		name: 'AC3',
		path: '../../img/proyectos/CHECOR_MAGNOLIAS/AC3.webp',
		classnames: '',
	  },
	  {
		name: 'AC4',
		path: '../../img/proyectos/CHECOR_MAGNOLIAS/AC4.webp',
		classnames: '',
	  },
	  {
		name: 'AC6',
		path: '../../img/proyectos/CHECOR_MAGNOLIAS/AC6(1).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel12 = {
	logo: '../../public/img/marcas/CHECOR.png',
	images: [
	  {
		name: 'Bike Parking',
		path: '../../img/proyectos/CHECOR_PORTA/BIKEPARKING.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina Frontal Porta',
		path: '../../img/proyectos/CHECOR_PORTA/COCINAFRONTAL_PORTA(1).webp',
		classnames: '',
	  },
	  {
		name: 'Cocina Integrada Porta',
		path: '../../img/proyectos/CHECOR_PORTA/COCINAINTEGRADA_PORTA_.webp',
		classnames: '',
	  },
	  {
		name: 'Diurna',
		path: '../../img/proyectos/CHECOR_PORTA/DIURNA(1).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/CHECOR_PORTA/DORMITORIOPRINCIPAL(5).webp',
		classnames: '',
	  },
	  {
		name: 'Estar X01',
		path: '../../img/proyectos/CHECOR_PORTA/ESTARX01.webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/CHECOR_PORTA/LOBBY(4).webp',
		classnames: '',
	  },
	  {
		name: 'Nocturna',
		path: '../../img/proyectos/CHECOR_PORTA/NOCTURNA(1).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor X01',
		path: '../../img/proyectos/CHECOR_PORTA/SALACOMEDORX01.webp',
		classnames: '',
	  },
	  {
		name: 'Terraza',
		path: '../../img/proyectos/CHECOR_PORTA/TERRAZA(3).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel42 = {
	logo: '',
	images: [
	  {
		name: 'Dormitorio',
		path: '../../img/proyectos/CONFORTA INMOBILIARIA_AMARA/DORMITORIO(4).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/CONFORTA INMOBILIARIA_AMARA/LOBBY(10).webp',
		classnames: '',
	  },
	  {
		name: 'Pet Zone',
		path: '../../img/proyectos/CONFORTA INMOBILIARIA_AMARA/PETZONE(1).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/CONFORTA INMOBILIARIA_AMARA/SALACOMEDOR(9).webp',
		classnames: '',
	  },
	  {
		name: 'Skyroom',
		path: '../../img/proyectos/CONFORTA INMOBILIARIA_AMARA/SKYROOM.webp',
		classnames: '',
	  },
	  {
		name: 'Terraza Lounge',
		path: '../../img/proyectos/CONFORTA INMOBILIARIA_AMARA/TERRAZALOUNGE(1).webp',
		classnames: '',
	  },
	  {
		name: 'Zona Bike',
		path: '../../img/proyectos/CONFORTA INMOBILIARIA_AMARA/ZONABIKE.webp',
		classnames: '',
	  },
	  {
		name: 'Zona de Yoga',
		path: '../../img/proyectos/CONFORTA INMOBILIARIA_AMARA/ZONADEYOGA.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel48 = {
	logo: '',
	images: [
	  {
		name: 'C1',
		path: '../../img/proyectos/DUOMA_LACORUÑA/C1.png',
		classnames: '',
	  },
	  {
		name: 'L2',
		path: '../../img/proyectos/DUOMA_LACORUÑA/L2.png',
		classnames: '',
	  },
	  {
		name: 'S1',
		path: '../../img/proyectos/DUOMA_LACORUÑA/S1.webp',
		classnames: '',
	  },
	  {
		name: 'S4',
		path: '../../img/proyectos/DUOMA_LACORUÑA/S4.png',
		classnames: '',
	  },
	]
  };
  
  
  
  const carousel8 = {
	logo: '../../public/img/marcas/EDIFICA.png',
	images: [
	  {
		name: 'Dpto 101 Dormitorio',
		path: '../../img/proyectos/EDIFICA_PALACIOS/DPTO101DORMITORIO.webp',
		classnames: '',
	  },
	  {
		name: 'Dpto 101 Sala',
		path: '../../img/proyectos/EDIFICA_PALACIOS/DPTO101SALA.webp',
		classnames: '',
	  },
	  {
		name: 'Dpto X2 Dormitorio Principal',
		path: '../../img/proyectos/EDIFICA_PALACIOS/DPTOX2DORMITORIOPRINCIPAL(2).webp',
		classnames: '',
	  },
	  {
		name: 'Dpto X2 Sala Comedor',
		path: '../../img/proyectos/EDIFICA_PALACIOS/DPTOX2SALACOMEDOR.webp',
		classnames: '',
	  },
	  {
		name: 'Lobby 1',
		path: '../../img/proyectos/EDIFICA_PALACIOS/LOBBY1(4).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby 2',
		path: '../../img/proyectos/EDIFICA_PALACIOS/LOBBY2(3).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel20 = {
	logo: '',
	images: [
	  {
		name: 'BBQ 03',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/BBQ03.webp',
		classnames: '',
	  },
	  {
		name: 'BBQ 04',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/BBQ04(1).webp',
		classnames: '',
	  },
	  {
		name: 'BBQ 02',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/BBQ_02.webp',
		classnames: '',
	  },
	  {
		name: 'Coworking',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/COWORKING.webp',
		classnames: '',
	  },
	  {
		name: 'Estacionamiento Bicicletas',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/ESTACIONAMIENTOBICICLETAS.webp',
		classnames: '',
	  },
	  {
		name: 'Gym',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/GYM(4).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/LOBBY(1).webp',
		classnames: '',
	  },
	  {
		name: 'Patio Interior',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/PATIOINTERIOR(1).webp',
		classnames: '',
	  },
	  {
		name: 'Pet Zone',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/PETZONE(2).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Gourmet',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/SALONGOURMET.webp',
		classnames: '',
	  },
	  {
		name: 'SUM 02',
		path: '../../img/proyectos/EDIFIKARTE_AREQUIPA AVENUE 4015/SUM02(1).webp',
		classnames: '',
	  },
	]
  };
  
  
  const carousel22 = {
	logo: '',
	images: [
	  {
		name: 'Baño',
		path: '../../img/proyectos/GRACIA_GEMA/BANO.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio (DOMR)',
		path: '../../img/proyectos/GRACIA_GEMA/DOMR.webp',
		classnames: '',
	  },
	  {
		name: 'Estar Juegos',
		path: '../../img/proyectos/GRACIA_GEMA/ESTAR_JUEGOS(1).webp',
		classnames: '',
	  },
	  {
		name: 'Oficina Gym',
		path: '../../img/proyectos/GRACIA_GEMA/OFI_GYM.webp',
		classnames: '',
	  },
	  {
		name: 'Patio',
		path: '../../img/proyectos/GRACIA_GEMA/PATIO(3).webp',
		classnames: '',
	  },
	  {
		name: 'Sala',
		path: '../../img/proyectos/GRACIA_GEMA/SALA(8).webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Estar',
		path: '../../img/proyectos/GRACIA_GEMA/SALADEESTAR(1).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Nocturna',
		path: '../../img/proyectos/GRACIA_GEMA/SALANOCTURNA.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel10 = {
	logo: '../../public/img/marcas/HAUSBRIK.png',
	images: [
	  {
		name: 'Baños 401 - 2',
		path: '../../img/proyectos/HAUSBRIK_MONTE ALEGRE/BAÑOS401_2.webp',
		classnames: '',
	  },
	  {
		name: 'Baños 401 - 3',
		path: '../../img/proyectos/HAUSBRIK_MONTE ALEGRE/BAÑOS401_3.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina Final en Alta',
		path: '../../img/proyectos/HAUSBRIK_MONTE ALEGRE/COCINAFINALENALTA.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal 401',
		path: '../../img/proyectos/HAUSBRIK_MONTE ALEGRE/DORMPRINC401.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Secundario 401',
		path: '../../img/proyectos/HAUSBRIK_MONTE ALEGRE/DORMSEC_401MA.webp',
		classnames: '',
	  },
	  {
		name: 'Frontal',
		path: '../../img/proyectos/HAUSBRIK_MONTE ALEGRE/FRONTAL(1).webp',
		classnames: '',
	  },
	  {
		name: 'Lateral',
		path: '../../img/proyectos/HAUSBRIK_MONTE ALEGRE/LATERAL(1).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/HAUSBRIK_MONTE ALEGRE/LOBBY(12).webp',
		classnames: '',
	  },
	  {
		name: 'Parque Interior',
		path: '../../img/proyectos/HAUSBRIK_MONTE ALEGRE/PARQUEINTERIOR.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel11 = {
	logo: '../../public/img/marcas/I.M.HENDERSON.png',
	images: [
	  {
		name: 'Baño Principal',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/BAÑOPRINC(1).webp',
		classnames: '',
	  },
	  {
		name: 'Cocina 11',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/COCINA(11).webp',
		classnames: '',
	  },
	  {
		name: 'Cocina 12',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/COCINA(12).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal 6',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/DORMITORIOPRINCIPAL(6).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal 7',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/DORMITORIOPRINCIPAL(7).webp',
		classnames: '',
	  },
	  {
		name: 'Frontal 169',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/FRONTAL169(1).webp',
		classnames: '',
	  },
	  
	  {
		name: 'Piscina ',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/PISCINA(13).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor 10',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/SALACOMEDOR(10).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor 11',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/SALACOMEDOR(11).webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Estar 2',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/SALADEESTAR(2).webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Estar 3',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/SALADEESTAR(3).webp',
		classnames: '',
	  },
	  {
		name: 'V2',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/V2(5).webp',
		classnames: '',
	  },
	  {
		name: 'V2 Diurna',
		path: '../../img/proyectos/IM HENDERSON_JACINTO DE LARA/V2DIURNA(1).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel26 = {
	logo: '../../public/img/marcas/INARCO.png',
	images: [
	  {
		name: 'Baño Principal',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/BAÑOPRINCIPAL(1).webp',
		classnames: '',
	  },
	  {
		name: 'Cafetería',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/CAFETERIA_MAGALLANES.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina 13',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/COCINA(13).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/DORMPRINCIPAL(3).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Secundario',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/DORMSECUNDARIO(2).webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Diurna',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/FACHADADIURNA(1).webp',
		classnames: '',
	  },
	  {
		name: 'Gourmet',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/GOURMET_MAGALLANES.webp',
		classnames: '',
	  },
	  {
		name: 'Gym',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/GYM_MAGALLANES.webp',
		classnames: '',
	  },
	  {
		name: 'Lounge',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/LOUNGE_MAGALLANES.webp',
		classnames: '',
	  },
	  {
		name: 'Minimarket',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/MINIMARKET(1).webp',
		classnames: '',
	  },
	  {
		name: 'Sala',
		path: '../../img/proyectos/INARCO PARAGUAY_MAGALLANES/SALA(9).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel30 = {
	logo: '',
	images: [
	  {
		name: 'Bike Parking',
		path: '../../img/proyectos/INMGENIO_INSERT/BIKEPARKING_.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina',
		path: '../../img/proyectos/INMGENIO_INSERT/COCINA.webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/INMGENIO_INSERT/DORMPRINC.webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Lateral',
		path: '../../img/proyectos/INMGENIO_INSERT/FACHADALATERAL(2).webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Esquinada',
		path: '../../img/proyectos/INMGENIO_INSERT/FACHADA__ESQUINADA(1).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby Insert',
		path: '../../img/proyectos/INMGENIO_INSERT/LOBBYINSERT.webp',
		classnames: '',
	  },
	  {
		name: 'Sala',
		path: '../../img/proyectos/INMGENIO_INSERT/SALA.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel36 = {
	logo: '../../public/img/marcas/LIBRE.png',
	images: [
	  {
		name: 'Terraza',
		path: '../../img/proyectos/LIBRE INMB_28 DE JULIO/6TERRAZA_.webp',
		classnames: '',
	  },
	  {
		name: 'Patio',
		path: '../../img/proyectos/LIBRE INMB_28 DE JULIO/7PATIO_.webp',
		classnames: '',
	  },
	  {
		name: 'Comedor',
		path: '../../img/proyectos/LIBRE INMB_28 DE JULIO/COMEDOR(5).webp',
		classnames: '',
	  },
	  {
		name: 'Fachada',
		path: '../../img/proyectos/LIBRE INMB_28 DE JULIO/FACHADA(2)(1).webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Frontal',
		path: '../../img/proyectos/LIBRE INMB_28 DE JULIO/FACHADAFRONTAL(2)(1).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/LIBRE INMB_28 DE JULIO/LOBBY(2)(1).webp',
		classnames: '',
	  },
	  {
		name: 'Sala',
		path: '../../img/proyectos/LIBRE INMB_28 DE JULIO/SALA(10).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel46 = {
	logo: '../../public/img/marcas/LIBRE.png',
	images: [
	  {
		name: 'F6',
		path: '../../img/proyectos/LIBRE INMOBILIARIA_MMXX/F6.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel18 = {
	logo: '../../public/img/marcas/linea_21.png',
	images: [
	  {
		name: 'Fachada',
		path: '../../img/proyectos/LINEA 21_FLORA 21/FACHADA(7).webp',
		classnames: '',
	  },
	  {
		name: 'Ingreso',
		path: '../../img/proyectos/LINEA 21_FLORA 21/INGRESO(2).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/LINEA 21_FLORA 21/LOBBY(13).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/LINEA 21_FLORA 21/SALACOMEDOR(12).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor 2',
		path: '../../img/proyectos/LINEA 21_FLORA 21/SALACOMEDOR2.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor 3',
		path: '../../img/proyectos/LINEA 21_FLORA 21/SALACOMEDOR3.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor 4',
		path: '../../img/proyectos/LINEA 21_FLORA 21/SALACOMEDOR4.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel52 = {
	logo: '../../public/img/marcas/linea_21.png',
	images: [
	  {
		name: 'Sala 80m2',
		path: '../../img/proyectos/LINEA 21_INCLÁN21/SALA80M2.webp',
		classnames: '',
	  },
	  {
		name: 'Sala 39mts',
		path: '../../img/proyectos/LINEA 21_INCLÁN21/SALA_39MTS.webp',
		classnames: '',
	  },
	  {
		name: 'Sala 702',
		path: '../../img/proyectos/LINEA 21_INCLÁN21/SALA_702.webp',
		classnames: '',
	  },
	  {
		name: 'Semisótano',
		path: '../../img/proyectos/LINEA 21_INCLÁN21/SEMISOTANO_INCLAN.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel28 = {
	logo: '../../public/img/marcas/linea_21.png',
	images: [
	  {
		name: 'C3',
		path: '../../img/proyectos/LINEA 21_PRADA21/C3.png',
		classnames: '',
	  },
	  {
		name: 'D3',
		path: '../../img/proyectos/LINEA 21_PRADA21/D3.png',
		classnames: '',
	  },
	  {
		name: 'D5',
		path: '../../img/proyectos/LINEA 21_PRADA21/D5.png',
		classnames: '',
	  },
	  {
		name: 'Patio',
		path: '../../img/proyectos/LINEA 21_PRADA21/PATIO(2).webp',
		classnames: '',
	  },
	  {
		name: 'S2',
		path: '../../img/proyectos/LINEA 21_PRADA21/S2.png',
		classnames: '',
	  },
	]
  };
  
  const carousel31 = {
	logo: '../../public/img/marcas/MARCAN.png',
	images: [
	  {
		name: 'Lobby',
		path: '../../img/proyectos/MARCAN_LLANO ZAPATA/LOBBY(6).webp',
		classnames: '',
	  },
	  {
		name: 'Patio 2',
		path: '../../img/proyectos/MARCAN_LLANO ZAPATA/PATIO(2).webp',
		classnames: '',
	  },
	  {
		name: 'Patio 3',
		path: '../../img/proyectos/MARCAN_LLANO ZAPATA/PATIO(3).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/MARCAN_LLANO ZAPATA/SALA COMEDOR 2.png',
		classnames: '',
	  },
	  {
		name: 'Sala 3',
		path: '../../img/proyectos/MARCAN_LLANO ZAPATA/SALA3.webp',
		classnames: '',
	  },
	  {
		name: 'Sala 4',
		path: '../../img/proyectos/MARCAN_LLANO ZAPATA/SALA4.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor 2',
		path: '../../img/proyectos/MARCAN_LLANO ZAPATA/SALACOMEDOR2.webp',
		classnames: '',
	  },
	  {
		name: 'Vista Exterior',
		path: '../../img/proyectos/MARCAN_LLANO ZAPATA/VISTAEXT.webp',
		classnames: '',
	  },
	  {
		name: 'Vista Exterior 1',
		path: '../../img/proyectos/MARCAN_LLANO ZAPATA/VISTAEXT1.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel5 = {
	logo: '../../public/img/marcas/MARCAN.png',
	images: [
	  {
		name: 'Auditorio',
		path: '../../img/proyectos/MARCAN_POD/AUDITORIO.webp',
		classnames: '',
	  },
	  {
		name: 'Comedor',
		path: '../../img/proyectos/MARCAN_POD/COMEDOR(6).webp',
		classnames: '',
	  },
	  {
		name: 'Estacionamiento Bicicletas',
		path: '../../img/proyectos/MARCAN_POD/ESTACIONAMIENTOBICICLETAS.webp',
		classnames: '',
	  },
	  {
		name: 'Frontal Final',
		path: '../../img/proyectos/MARCAN_POD/FRONTALFINAL.webp',
		classnames: '',
	  },
	  {
		name: 'Lateral Final',
		path: '../../img/proyectos/MARCAN_POD/LATERALFINAL.webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/MARCAN_POD/LOBBY(14).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby Vivienda Alta',
		path: '../../img/proyectos/MARCAN_POD/LOBBYVIVIENDAALTA.webp',
		classnames: '',
	  },
	  {
		name: 'Pet Zone',
		path: '../../img/proyectos/MARCAN_POD/PETZONE.webp',
		classnames: '',
	  },
	  {
		name: 'Piscina',
		path: '../../img/proyectos/MARCAN_POD/PISCINA(13).webp',
		classnames: '',
	  },
	  {
		name: 'Plaza de Ingreso',
		path: '../../img/proyectos/MARCAN_POD/PLAZADEINGRESO.webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Juegos',
		path: '../../img/proyectos/MARCAN_POD/SALADEJUEGOS.webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Reuniones',
		path: '../../img/proyectos/MARCAN_POD/SALADEREUNIONES.webp',
		classnames: '',
	  },
	  {
		name: 'Terraza',
		path: '../../img/proyectos/MARCAN_POD/TERRAZA(6).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel25 = {
	logo: '../../public/img/marcas/MARCAN.png',
	images: [
	  {
		name: 'R6000 SUM',
		path: '../../img/proyectos/MARCAN_R6000/210916R6000SUM.webp',
		classnames: '',
	  },
	  {
		name: 'R6000 SJ01',
		path: '../../img/proyectos/MARCAN_R6000/210917R6000SJ01.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Reuniones',
		path: '../../img/proyectos/MARCAN_R6000/210917SALAREUNIONES.webp',
		classnames: '',
	  },
	  {
		name: 'F9',
		path: '../../img/proyectos/MARCAN_R6000/F9.webp',
		classnames: '',
	  },
	  {
		name: 'Montaje Final',
		path: '../../img/proyectos/MARCAN_R6000/MONTAJEFINAL.webp',
		classnames: '',
	  },
	  {
		name: 'R6000 Final',
		path: '../../img/proyectos/MARCAN_R6000/R6000FINAL.webp',
		classnames: '',
	  },
	  {
		name: 'SUM Primer Piso',
		path: '../../img/proyectos/MARCAN_R6000/SUMPRIMERPISO.webp',
		classnames: '',
	  },
	  {
		name: 'Terraza Piso 8',
		path: '../../img/proyectos/MARCAN_R6000/TERRAZAPISO8.webp',
		classnames: '',
	  },
	  {
		name: 'Terraza Sala Reuniones',
		path: '../../img/proyectos/MARCAN_R6000/TERRAZASALAREUNIONES.webp',
		classnames: '',
	  },
	  {
		name: 'Terraza Sala Reuniones Interior',
		path: '../../img/proyectos/MARCAN_R6000/TERRAZASALAREUNIONESINTERIOR.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel45 = {
	logo: '../../public/img/marcas/MODULAR.jpg',
	images: [
	  {
		name: 'Mendiburú',
		path: '../../img/proyectos/MODULAR_MENDIBURÚ/MENDIBURÚ.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel4 = {
	logo: '../../public/img/marcas/MORADA.png',
	images: [
	  {
		name: 'Morada Baño Principal',
		path: '../../img/proyectos/MORADA_PARDO664/MORADA_BAÑOPRINC(1).webp',
		classnames: '',
	  },
	  {
		name: 'Morada Cocina',
		path: '../../img/proyectos/MORADA_PARDO664/MORADA_COCINA(1).webp',
		classnames: '',
	  },
	  {
		name: 'Morada Dormitorio Principal',
		path: '../../img/proyectos/MORADA_PARDO664/MORADA_DORMITORIOPRINCIPAL(1).webp',
		classnames: '',
	  },
	  {
		name: 'Morada Dormitorio Secundario',
		path: '../../img/proyectos/MORADA_PARDO664/MORADA_DORMSEC(1).webp',
		classnames: '',
	  },
	  {
		name: 'Morada Sala Comedor',
		path: '../../img/proyectos/MORADA_PARDO664/MORADA_SALACOMEDOR(2).webp',
		classnames: '',
	  },
	]
  };
  
  
  
  const carousel27 = {
	logo: '../../public/img/marcas/NOVABILDIN.png',
	images: [
	  {
		name: 'AC5',
		path: '../../img/proyectos/NOVABILDIN_HENKO/AC5.webp',
		classnames: '',
	  },
	  {
		name: 'Departamento',
		path: '../../img/proyectos/NOVABILDIN_HENKO/DPTO.webp',
		classnames: '',
	  },
	  {
		name: 'Estacionamiento Bicicletas',
		path: '../../img/proyectos/NOVABILDIN_HENKO/ESTACIONAMIENTOBICICLETAS.webp',
		classnames: '',
	  },
	  {
		name: 'F2',
		path: '../../img/proyectos/NOVABILDIN_HENKO/F2.webp',
		classnames: '',
	  },
	  {
		name: 'Henkonoche',
		path: '../../img/proyectos/NOVABILDIN_HENKO/HENKONOCHE_.webp',
		classnames: '',
	  },
	  {
		name: 'Henkos',
		path: '../../img/proyectos/NOVABILDIN_HENKO/HENKOS.webp',
		classnames: '',
	  },
	  {
		name: 'Jardín Seco',
		path: '../../img/proyectos/NOVABILDIN_HENKO/JARDINSECO.webp',
		classnames: '',
	  },
	  {
		name: 'L1',
		path: '../../img/proyectos/NOVABILDIN_HENKO/L1.webp',
		classnames: '',
	  },
	  {
		name: 'Piscina',
		path: '../../img/proyectos/NOVABILDIN_HENKO/PISCINA(11).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel51 = {
	logo: '../../public/img/marcas/NOVABILDIN.png',
	images: [
	  {
		name: 'Diurna',
		path: '../../img/proyectos/NOVABILDIN_SONHOS/DIURNA.webp',
		classnames: '',
	  },
	  {
		name: 'Nocturna',
		path: '../../img/proyectos/NOVABILDIN_SONHOS/NOCTURNA(1).webp',
		classnames: '',
	  },
	  {
		name: 'Sonhos',
		path: '../../img/proyectos/NOVABILDIN_SONHOS/SONHOS.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel29 = {
	logo: '../../public/img/marcas/OMNINVEST.png',
	images: [
	  {
		name: 'Vista Exterior',
		path: '../../img/proyectos/OMNIVEST_RESIDENZA CASTAGNI/VISTAEXTERIOR(4).webp',
		classnames: '',
	  },
	  {
		name: 'Vista Interior 1',
		path: '../../img/proyectos/OMNIVEST_RESIDENZA CASTAGNI/Vistainterna1(1).webp',
		classnames: '',
	  },
	  {
		name: 'Vista Interior 2',
		path: '../../img/proyectos/OMNIVEST_RESIDENZA CASTAGNI/Vistainterna2(1).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel53 = {
	logo: '../../public/img/marcas/PUENTE.jpg',
	images: [
	  {
		name: 'Corredor',
		path: '../../img/proyectos/PUENTE STUDIO_OXAPAMPA/CORREDOR.webp',
		classnames: '',
	  },
	  {
		name: 'Exterior 1',
		path: '../../img/proyectos/PUENTE STUDIO_OXAPAMPA/EXTERIOR1.webp',
		classnames: '',
	  },
	  {
		name: 'Exterior 2',
		path: '../../img/proyectos/PUENTE STUDIO_OXAPAMPA/EXTERIOR2.webp',
		classnames: '',
	  },
	  {
		name: 'Frontal',
		path: '../../img/proyectos/PUENTE STUDIO_OXAPAMPA/FRONTAL.webp',
		classnames: '',
	  },
	  {
		name: 'Interior 1',
		path: '../../img/proyectos/PUENTE STUDIO_OXAPAMPA/INTERIOR1.webp',
		classnames: '',
	  },
	  {
		name: 'Interior 2',
		path: '../../img/proyectos/PUENTE STUDIO_OXAPAMPA/INTERIOR2.webp',
		classnames: '',
	  },
	  {
		name: 'Interior 3',
		path: '../../img/proyectos/PUENTE STUDIO_OXAPAMPA/INTERIOR3.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel41 = {
	logo: '',
	images: [
	  {
		name: 'D5 Image56',
		path: '../../img/proyectos/QATA_GTD/D5_Image56_20230223_073630.webp',
		classnames: '',
	  },
	  {
		name: 'D5 Image57',
		path: '../../img/proyectos/QATA_GTD/D5_Image57_20230223_073231.webp',
		classnames: '',
	  },
	  {
		name: 'D5 Image58',
		path: '../../img/proyectos/QATA_GTD/D5_Image58_20230223_072833(1).webp',
		classnames: '',
	  },
	  {
		name: 'D5 Image62',
		path: '../../img/proyectos/QATA_GTD/D5_Image62_20230223_071154.webp',
		classnames: '',
	  },
	  {
		name: 'D5 Image67',
		path: '../../img/proyectos/QATA_GTD/D5_Image67_20230223_065405.webp',
		classnames: '',
	  },
	  {
		name: 'D5 Image71',
		path: '../../img/proyectos/QATA_GTD/D5_Image71_20230223_063908.webp',
		classnames: '',
	  },
	  {
		name: 'D5 Image84',
		path: '../../img/proyectos/QATA_GTD/D5_Image84_20230223_103653.webp',
		classnames: '',
	  },
	  {
		name: 'D5 Image85',
		path: '../../img/proyectos/QATA_GTD/D5_Image85_20230223_103332.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel21 = {
	logo: '../../public/img/marcas/emme_inmob.png',
	images: [
	  // No se listaron imágenes en esta carpeta
	]
  };
  
  const carousel14 = {
	logo: '../../public/img/marcas/QUADRAT.png',
	images: [
	  {
		name: 'Cocina Duplex',
		path: '../../img/proyectos/QUADRAT_SILICE/COCINADUPLEX_PUMACAHUA.webp',
		classnames: '',
	  },
	  {
		name: 'Cocina 75mts',
		path: '../../img/proyectos/QUADRAT_SILICE/COCINA_75MTS(1).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal 65mts',
		path: '../../img/proyectos/QUADRAT_SILICE/DORMITORIO_PRINCIPAL65MTS(2).webp',
		classnames: '',
	  },
	  {
		name: 'Dorm Niño',
		path: '../../img/proyectos/QUADRAT_SILICE/DORMNIÑO_PUMACAHUA.webp',
		classnames: '',
	  },
	  {
		name: 'Dorm Principal',
		path: '../../img/proyectos/QUADRAT_SILICE/DORMPRINC_PUMACAHUA_(1).webp',
		classnames: '',
	  },
	  {
		name: 'Sala 75mts',
		path: '../../img/proyectos/QUADRAT_SILICE/SALA75MTS_PUMACAHUA.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor 65mts',
		path: '../../img/proyectos/QUADRAT_SILICE/SALACOMEDOR65MTS.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/QUADRAT_SILICE/SALACOMEDOR_PUMACAHUA(3).webp',
		classnames: '',
	  },
	  {
		name: 'Terraza',
		path: '../../img/proyectos/QUADRAT_SILICE/TERRAZA(7).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel6 = {
	logo: '../../public/img/marcas/rafi_segal.png',
	images: [
	  {
		name: 'ISO',
		path: '../../img/proyectos/RAFI SEGAL_CAREHOUSE CHICAGO/ISO(2).webp',
		classnames: '',
	  },
	  {
		name: 'V1',
		path: '../../img/proyectos/RAFI SEGAL_CAREHOUSE CHICAGO/V1.webp',
		classnames: '',
	  },
	  {
		name: 'V2',
		path: '../../img/proyectos/RAFI SEGAL_CAREHOUSE CHICAGO/V2.webp',
		classnames: '',
	  },
	  {
		name: 'Vista Cocina',
		path: '../../img/proyectos/RAFI SEGAL_CAREHOUSE CHICAGO/VISTACOCINA(3).webp',
		classnames: '',
	  },
	  {
		name: 'Vista Patio',
		path: '../../img/proyectos/RAFI SEGAL_CAREHOUSE CHICAGO/VISTAPATIO(4).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel37 = {
	logo: '',
	images: [
	  {
		name: 'Acercamiento',
		path: '../../img/proyectos/SAN CHARBEL_BOTANIKA/ACERCAMIENTO(3).webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Frontal',
		path: '../../img/proyectos/SAN CHARBEL_BOTANIKA/FACHADAFRONTAL.webp',
		classnames: '',
	  },
	  {
		name: 'Fachada Nocturna',
		path: '../../img/proyectos/SAN CHARBEL_BOTANIKA/FACHADANOCTURNA(1).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel49 = {
	logo: '',
	images: [
	  {
		name: 'Patio Interior',
		path: '../../img/proyectos/SOLAR INMOBILIARIA_SOLDADO CABADA 306/PATIOINTERIOR(1).webp',
		classnames: '',
	  },
	  {
		name: 'V1',
		path: '../../img/proyectos/SOLAR INMOBILIARIA_SOLDADO CABADA 306/V1(3).webp',
		classnames: '',
	  },
	  {
		name: 'V2',
		path: '../../img/proyectos/SOLAR INMOBILIARIA_SOLDADO CABADA 306/V2(5).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel39 = {
	logo: '../../public/img/marcas/TYC.png',
	images: [
	  {
		name: 'Baño Principal',
		path: '../../img/proyectos/TYC_NIUS/BAÑOPRINC_NIUS.webp',
		classnames: '',
	  },
	  {
		name: 'Coworking',
		path: '../../img/proyectos/TYC_NIUS/COWORKING_NIUS.webp',
		classnames: '',
	  },
	  {
		name: 'Diurna',
		path: '../../img/proyectos/TYC_NIUS/DIA(3).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/TYC_NIUS/DORMITORIOPRINCIPAL(8).webp',
		classnames: '',
	  },
	  {
		name: 'Gym',
		path: '../../img/proyectos/TYC_NIUS/GYM_NIUS.webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/TYC_NIUS/LOBBY_NIUS.webp',
		classnames: '',
	  },
	  {
		name: 'Nocturna',
		path: '../../img/proyectos/TYC_NIUS/NOCHE(2).webp',
		classnames: '',
	  },
	  {
		name: 'Parrilla',
		path: '../../img/proyectos/TYC_NIUS/PARRILLA_NIUS(1).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/TYC_NIUS/SALACOMEDOR_NIUS.webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Niños',
		path: '../../img/proyectos/TYC_NIUS/SALADENIÑOS_NIUS.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Gourmet',
		path: '../../img/proyectos/TYC_NIUS/SALAGOURMET_NIUS.webp',
		classnames: '',
	  },
	  {
		name: 'SUM',
		path: '../../img/proyectos/TYC_NIUS/SUM_NIUS.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel13 = {
	logo: '../../public/img/marcas/TYC.png',
	images: [
	  {
		name: 'Balcones',
		path: '../../img/proyectos/TYC_PARDO COAST/BALCONES.webp',
		classnames: '',
	  },
	  {
		name: 'Baño',
		path: '../../img/proyectos/TYC_PARDO COAST/BAÑO(5).webp',
		classnames: '',
	  },
	  {
		name: 'Cine',
		path: '../../img/proyectos/TYC_PARDO COAST/CINE.webp',
		classnames: '',
	  },
	  {
		name: 'Coworking',
		path: '../../img/proyectos/TYC_PARDO COAST/COWORKING(1).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio',
		path: '../../img/proyectos/TYC_PARDO COAST/DORMITORIO(6).webp',
		classnames: '',
	  },
	  {
		name: 'Estacionamiento Bicicletas',
		path: '../../img/proyectos/TYC_PARDO COAST/ESTACIONAMIENTOBICICLETAS(1).webp',
		classnames: '',
	  },
	  {
		name: 'Gym',
		path: '../../img/proyectos/TYC_PARDO COAST/GYM(5).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/TYC_PARDO COAST/LOBBY(16).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby Atmosfera',
		path: '../../img/proyectos/TYC_PARDO COAST/LOBBYATMOSFERA.webp',
		classnames: '',
	  },
	  {
		name: 'Parrilla',
		path: '../../img/proyectos/TYC_PARDO COAST/PARRILLA(3).webp',
		classnames: '',
	  },
	  {
		name: 'Paseo de Mascotas',
		path: '../../img/proyectos/TYC_PARDO COAST/PASEODEMASCOTAS.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Bar',
		path: '../../img/proyectos/TYC_PARDO COAST/SALABAR.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/TYC_PARDO COAST/SALACOMEDOR.webp',
		classnames: '',
	  },
	  {
		name: 'Sala Gourmet',
		path: '../../img/proyectos/TYC_PARDO COAST/SALAGOURMET.webp',
		classnames: '',
	  },
	  {
		name: 'SUM',
		path: '../../img/proyectos/TYC_PARDO COAST/SUM(2).webp',
		classnames: '',
	  },
	  {
		name: 'Terraza Parrilla',
		path: '../../img/proyectos/TYC_PARDO COAST/TERRAZAPARRILLA_.webp',
		classnames: '',
	  },
	  {
		name: 'Zona de Juegos',
		path: '../../img/proyectos/TYC_PARDO COAST/ZONAJUEGOS.webp',
		classnames: '',
	  },
	  {
		name: 'Zona Pet',
		path: '../../img/proyectos/TYC_PARDO COAST/ZONAPET.webp',
		classnames: '',
	  },
	  {
		name: 'Zoom Fachada',
		path: '../../img/proyectos/TYC_PARDO COAST/ZOOMFACHADA(1).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel7 = {
	logo: '',
	images: [
	  {
		name: 'Acercamiento',
		path: '../../img/proyectos/VIVE PLM_ALIAGA/ACERCAMIENTO(4).webp',
		classnames: '',
	  },
	  {
		name: 'Fachada General',
		path: '../../img/proyectos/VIVE PLM_ALIAGA/FACHADAGENERAL.webp',
		classnames: '',
	  },
	  {
		name: 'Lobby 1',
		path: '../../img/proyectos/VIVE PLM_ALIAGA/LOBBY1(2).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby 2',
		path: '../../img/proyectos/VIVE PLM_ALIAGA/LOBBY2(4).webp',
		classnames: '',
	  },
	  {
		name: 'Sala de Niños',
		path: '../../img/proyectos/VIVE PLM_ALIAGA/SALADENIÑOS(6).webp',
		classnames: '',
	  },
	  {
		name: 'Social Kitchen',
		path: '../../img/proyectos/VIVE PLM_ALIAGA/SOCIALKITCHEN.webp',
		classnames: '',
	  },
	]
  };
  
  const carousel15 = {
	logo: '',
	images: [
	  {
		name: 'Dormitorio Secundario',
		path: '../../img/proyectos/VIVE PLM_ETEN1/DORMITORIOSECUNDARIO(1).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio Principal',
		path: '../../img/proyectos/VIVE PLM_ETEN1/DORMPRINCETEN(2).webp',
		classnames: '',
	  },
	  {
		name: 'Lobby',
		path: '../../img/proyectos/VIVE PLM_ETEN1/LOBBY(17).webp',
		classnames: '',
	  },
	  {
		name: 'Sala Comedor',
		path: '../../img/proyectos/VIVE PLM_ETEN1/SALACOMEDOR(1).webp',
		classnames: '',
	  },
	  {
		name: 'Terraza',
		path: '../../img/proyectos/VIVE PLM_ETEN1/TERRAZA(1).webp',
		classnames: '',
	  },
	]
  };
  
  const carousel32 = {
	logo: '',
	images: [
	  {
		name: 'Cocina',
		path: '../../img/proyectos/YAIR MAYO_CASA MODELO/COCINA(15).webp',
		classnames: '',
	  },
	  {
		name: 'Cocina 2',
		path: '../../img/proyectos/YAIR MAYO_CASA MODELO/COCINA2(1).webp',
		classnames: '',
	  },
	  {
		name: 'Comedor',
		path: '../../img/proyectos/YAIR MAYO_CASA MODELO/COMEDOR(7).webp',
		classnames: '',
	  },
	  {
		name: 'Dormitorio',
		path: '../../img/proyectos/YAIR MAYO_CASA MODELO/DORMITORIO(7).webp',
		classnames: '',
	  },
	  {
		name: 'Sala',
		path: '../../img/proyectos/YAIR MAYO_CASA MODELO/SALA(12).webp',
		classnames: '',
	  },
	  {
		name: 'Terraza',
		path: '../../img/proyectos/YAIR MAYO_CASA MODELO/TERRAZA(8).webp',
		classnames: '',
	  },
	  {
		name: 'Terraza con Poltrona',
		path: '../../img/proyectos/YAIR MAYO_CASA MODELO/TERRAZA_POLTRONAS(2).webp',
		classnames: '',
	  },
	]
  };
  

const Maqueta = {
	logo: '../../img/marcas/MARCAN.jpg',
	images: [
		{
			name: '1',
			path: '../../img/proyectos/maqueta/1.webp',
			classnames: '',
		},
		{
			name: '2',
			path: '../../img/proyectos/maqueta/2.webp',
			classnames: '',
		},
		{
			name: '3',
			path: '../../img/proyectos/maqueta/3.webp',
			classnames: '',
		},
	],
}

const carousels = {
	carousel1,
	carousel2,
	carousel3,
	carousel4,
	carousel5,
	carousel6,
	carousel7,
	carousel8,
	carousel9,
	carousel10,
	carousel11,
	carousel12,
	carousel13,
	carousel14,
	carousel15,
	carousel16,
	carousel17,
	carousel18,
	carousel19,
	carousel20,
	carousel21,
	carousel22,
	carousel23,
	carousel24,
	carousel25,
	carousel26,
	carousel27,
	carousel28,
	carousel29,
	carousel30,
	carousel31,
	carousel32,
	carousel33,
	carousel34,
	carousel35,
	carousel36,
	carousel37,
	carousel38,
	carousel39,
	carousel40,
	carousel41,
	carousel42,
	carousel43,
	carousel44,
	carousel45,
	carousel46,
	carousel47,
	carousel48,
	carousel49,
	carousel50,
	carousel51,
	carousel52,
	carousel53

}

export default carousels
